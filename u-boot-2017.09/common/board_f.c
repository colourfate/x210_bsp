...
#endif
	/* uboot重定位的地址在靠近DRAM顶部的一个区域，当重定位后，链接地址和实际地址又不一致了
	 * 但是在编译uboot的时候默认是打开了-fpic选项的，也就是c语言汇编后的代码全是位置无关码
	 * 也就不用担心链接地址和实际地址不一致了
	 * 这里relocaddr=0x4FF5F000，SDRAM末尾是0x50000000，也就是给uboot留了0xA1000=644KB的空间，
	 * 这是完全够的，uboot这么做的目的是给kennel留出空间 */
